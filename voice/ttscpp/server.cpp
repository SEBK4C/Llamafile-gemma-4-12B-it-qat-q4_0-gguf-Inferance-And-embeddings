...
// The Kokoro runner aborts the whole process on degenerate prompts:
// whitespace-only input (empty phoneme sequence) and any single sentence
// past its sequence limit (empirically dies between 300 and 600 chars
// without a . ! or ? — commas and semicolons do not split). Unbroken digit
// runs hit the same limit once expanded to words. Normalize instead of
// crashing.
static std::string sanitize_tts_prompt(const std::string & in) {
    // trim; whitespace-only becomes empty (handler turns that into a 400)
    size_t b = in.find_first_not_of(" \t\r\n\f\v");
    if (b == std::string::npos) {
        return "";
    }
    size_t e = in.find_last_not_of(" \t\r\n\f\v");
    // break digit runs into groups of 8
    std::string s;
    s.reserve(e - b + 16);
    int run = 0;
    ...
    // force a sentence terminator at least every SENT_MAX chars
    const size_t SENT_MAX = 220;
    std::string out;
    out.reserve(s.size() + s.size() / SENT_MAX * 2 + 2);
    size_t since = 0;
    size_t last_space = std::string::npos; // index in `out` of last space
    for (size_t i = 0; i < s.size(); i++) {
        ...
        if (since >= SENT_MAX) {
            if (last_space != std::string::npos) {
                out[last_space] = '.';
                out.insert(last_space + 1, 1, ' ');
                since = out.size() - last_space - 2;
            } else if (i + 1 >= s.size() || ((unsigned char)s[i + 1] & 0xC0) != 0x80) {
                // no space for 220 chars (one giant token): only cut at a
...
int main(int argc, const char ** argv) {
    ...
    const auto handle_tts = [
        ...
        prompt = sanitize_tts_prompt(prompt);
        if (prompt.empty()) {
