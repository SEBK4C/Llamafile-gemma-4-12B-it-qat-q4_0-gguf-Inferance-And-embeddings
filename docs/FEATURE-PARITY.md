...
| 18 | Hardware autotune (v0.3, `LLAMAFILE_NO_AUTOTUNE`) | bare run on odd hardware | ✅ **RESTORED in v0.7.1-universal, acceptance-verified (07-07)** — fresh HF artifact (commit `9d09cff`, sha256 `d911648e…65d52`), clean extraction dir, idle 12 GB 3080 Ti: bare `sh ./gemma4-server.llamafile --gpu nvidia` boots clean. Evidence: stderr `autotune: CUDA detected — applied tuned defaults (-np 1 -c 131072 -b 2048 -ub 256 -ctk f16 -ctv f16 -sm none)`; /health 200 in ~4 s; **11,008 MiB used / 903 MiB VRAM headroom**; no OOM/segfault/alloc failures in the boot log; chat ~126–139 tok/s; `/v1/embeddings` 1024-dim; `/embed/health` 200. (v0.7.0 regression: `n_parallel auto = 4`, no ctx cap → CUDA0 alloc fail + segfault on 12 GB.) Autotune restored via fork `args.cpp` on branch `v0.7.x-universal` @ 093b376 | ✅ autotune verified firing on Metal tier (07-07 Mac rebuild); `.args.xnu` prewarm also verified |
...
| 2026-07-07 | CUDA (PVE host, 3080 Ti) — **v0.7.1-universal HF artifact, row-18 acceptance** | **BARE-BOOT PASS** (targeted run: autotune + health + chat + embeddings + /embed/health) | fresh download (HF `9d09cff`, sha256-verified), clean extraction dir; autotune line fired (`-np 1 -c 131072 -b 2048 -ub 256 -ctk f16 -ctv f16 -sm none`); /health 200 ~4 s; 903 MiB VRAM headroom; chat 126–139 tok/s (MTP accept 0.55–0.62); embeddings 1024-dim; /embed/health 200; prod pause 98 s total |
